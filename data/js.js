...
var AVATARS=['\u{1F9D1}\u{200D}\u{1F393}','\u{1F468}\u{200D}\u{1F393}','\u{1F469}\u{200D}\u{1F393}','\u{1F9D1}\u{200D}\u{1F3EB}','\u{1F468}\u{200D}\u{1F4BC}','\u{1F469}\u{200D}\u{1F4BC}','\u{1F468}\u{200D}\u{2695}\u{FE0F}','\u{1F469}\u{200D}\u{2695}\u{FE0F}','\u{1F468}\u{200D}\u{1F680}','\u{1F469}\u{200D}\u{1F680}','\u{1F468}\u{200D}\u{1F3A4}','\u{1F469}\u{200D}\u{1F3A4}'];
var AVATARS_GIF=[{emoji:'\u{1F466}\u{1F3FB}',label:'BOY',cls:'gif'},{emoji:'\u{1F467}\u{1F3FB}',label:'GIRL',cls:'gif'}];
...
var I18N={en:{welcome:'Welcome Back',enter_idpin:'Sign in to continue',sign_in:'SIGN IN',learn:'Learn',practice:'Practice',speak:'Speak',extra:'Extra',welcome_to:'Welcome to',digital:'Digital',book:'Book',tagline:'Learning English is <b>easy</b>, <b>fun</b>, and <b>enjoyable!</b>'},id:{welcome:'Selamat Datang',enter_idpin:'Masuk untuk lanjut',sign_in:'MASUK',learn:'Belajar',practice:'Latihan',speak:'Bicara',extra:'Ekstra',welcome_to:'Selamat datang di',digital:'Digital',book:'Buku',tagline:'Belajar bahasa Inggris itu <b>mudah</b>, <b>seru</b>, dan <b>menyenangkan!</b>'}};
...
function setGreet(){var now=new Date();var h=now.getHours();var greetEn=h>=4&&h<12?'Good morning':h<15?'Good afternoon':h<19?'Good evening':'Good night';var greetId=h>=4&&h<12?'Selamat pagi':h<15?'Selamat siang':h<19?'Selamat sore':'Selamat malam';var g=lang==='id'?greetId:greetEn;var gEl=$('greet');if(gEl)gEl.textContent=g+'!';}
function updateTime(){var now=new Date();var days=lang==='id'?DAYS_ID:DAYS_EN;var months=lang==='id'?MONTHS_ID:MONTHS_EN;var day=days[now.getDay()];var dd=pad2(now.getDate());var month=months[now.getMonth()];var yyyy=now.getFullYear();var h24=now.getHours();var ampm=h24>=12?'PM':'AM';var h12=h24%12;if(h12===0)h12=12;var hh=pad2(h12);var mm=pad2(now.getMinutes());var ss=pad2(now.getSeconds());var str=day+', '+dd+' '+month+' '+yyyy+' | '+hh+':'+mm+':'+ss+' '+ampm;var el=$('time-display');if(el)el.textContent=str;}
...
var focusMode=false;
function startFocusGuard(){focusMode=true;try{history.pushState({focus:1},'');}catch(e){}window.addEventListener('popstate',onFocusBack);window.addEventListener('beforeunload',onBeforeUnload);}
function stopFocusGuard(){focusMode=false;window.removeEventListener('popstate',onFocusBack);window.removeEventListener('beforeunload',onBeforeUnload);}
function onFocusBack(){if(!focusMode)return;try{history.pushState({focus:1},'');}catch(e){}showLockedMsg('focus','Ayo tetap fokus belajar!','Jangan keluar dulu, selesaikan sesi fokusmu.');}
function onBeforeUnload(e){if(focusMode){e.preventDefault();e.returnValue='';}}
function showLockedMsg(title,msg,sub){$('locked-title').textContent=title||'Terkunci';$('locked-msg').innerHTML=esc(msg||'')+(sub?'<br><small>'+esc(sub)+'</small>':'');$('ov-locked').classList.add('on');}
...
async function fetchLeaderboardFromSheet(forceFresh){var now=Date.now();if(!forceFresh&&_lbCache&&(now-_lbCacheTime)<60000)return _lbCache;try{var url=(CFG.LOG||'')+'?action=fetch_leaderboard&_='+now;var r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);var data=await r.json();var arr=Array.isArray(data)?data:(data.rows||data.data||[]);_lbCache=arr;_lbCacheTime=now;return arr;}catch(e){ERR('fetchLeaderboard:',e.message);return _lbCache||[];}}
async function updateOnlineCount(){try{if(!CFG.LOG)return;var r=await fetch(CFG.LOG+'?action=fetch_online&minutes=5&_='+Date.now(),{cache:'no-store'});if(!r.ok)return;var arr=await r.json();var count=Array.isArray(arr)?arr.length:0;var el=$('online-num');if(el)el.textContent=count;}catch(e){}}
/* === END PART 1/4 === */
/* =====================================================
   AEC MV2 — js.js v5.3 FINAL
   PART 2/4: Data Loading, Storage, Achievements, Avatars, Router
   ===================================================== */
...
async function loadAll(forceFresh){if(forceFresh){['c_user','c_speaking','c_grammar','c_vocab','c_speaklive'].forEach(function(k){localStorage.removeItem(k);});D=null;_promise=null;_loadError=null;}if(D)return D;if(_promise)return _promise;showLoadingState();_promise=(async function(){var cu=cacheGet('user'),cs=cacheGet('speaking'),cg=cacheGet('grammar'),cv=cacheGet('vocab');if(cu&&cs&&cg&&cv){D={settings:cu.settings||{},students:cu.students||[],speaking:cs.materials||{},grammar:cg.materials||{},vocabMats:(cv.materials)||{},vocabWords:(cv.words)||[],speaklive:(cacheGet('speaklive')||{}).kemampuan||{A:[],B:[],C:[]}};setLogos(D);}var results=await Promise.allSettled([loadJSON('user',CFG.GH+'user.json'),loadJSON('speaking',CFG.GH+'speaking.json'),loadJSON('grammar',CFG.GH+'grammar.json'),loadJSON('vocab',CFG.GH+'vocab.json'),loadJSON('speaklive',CFG.GH+'speaklive.json')]);var u=results[0].status==='fulfilled'?results[0].value:null;var sp=results[1].status==='fulfilled'?results[1].value:null;var gr=results[2].status==='fulfilled'?results[2].value:null;var vc=results[3].status==='fulfilled'?results[3].value:null;var sl=results[4].status==='fulfilled'?results[4].value:null;if(!u)throw new Error('user.json gagal dimuat dari '+CFG.GH+'user.json');D={settings:u.settings||{},students:u.students||[],speaking:(sp&&sp.materials)||{},grammar:(gr&&gr.materials)||{},vocabMats:(vc&&vc.materials)||{},vocabWords:(vc&&vc.words)||[],speaklive:(sl&&sl.kemampuan)||{A:[],B:[],C:[]}};setLogos(D);return D;})().catch(function(e){_loadError=e;throw e;}).finally(function(){_promise=null;});return _promise;}
function setLogos(d){var la=(d.settings&&d.settings.logoAEC)||'',ls=(d.settings&&d.settings.logoSchool)||'';var s1=$('lg1'),s2=$('lg2'),h1=$('h-logo1'),h2=$('h-logo2');var l1=$('login-logo1'),l2=$('login-logo2');if(la){s1.src=la;s1.style.display='';h1.src=la;h1.style.display='';if(l1)l1.src=la;}else{s1.style.display='none';h1.style.display='none';}if(ls){s2.src=ls;s2.style.display='';h2.src=ls;h2.style.display='';if(l2)l2.src=ls;}else{s2.style.display='none';h2.style.display='none';}}
...
function allSentences(){var sents=[];if(!D)return sents;Object.values(D.speaking||{}).forEach(function(u){Object.values(u.parts||{}).forEach(function(p){buildSent(p.transcript||'').forEach(function(s){if(s&&s.length>5&&s.split(' ').length>=3)sents.push(s);});});});return sents;}
...
var ACH_LIST=[{id:'first',mi:'celebration',name:'First Step',d:'Login pertama kali'},{id:'part1',mi:'task_alt',name:'First Part',d:'Selesaikan 1 part materi'},{id:'part5',mi:'directions_run',name:'On Fire',d:'Selesaikan 5 part'},{id:'part10',mi:'flag',name:'Scholar',d:'Selesaikan 10 part'},{id:'part20',mi:'military_tech',name:'Master',d:'Selesaikan 20 part'},{id:'quiz1',mi:'quiz',name:'Quiz Starter',d:'Selesaikan quiz pertama'},{id:'quizA',mi:'star',name:'Quiz Master',d:'Nilai A di quiz'},{id:'vocab10',mi:'translate',name:'Word Hunter',d:'10 kata vocab dikuasai'},{id:'vocab50',mi:'auto_stories',name:'Word Master',d:'50 kata vocab dikuasai'},{id:'speak1',mi:'mic',name:'First Speech',d:'Speak live pertama'},{id:'speakA',mi:'record_voice_over',name:'Great Speaker',d:'Nilai A di speak'},{id:'bm1',mi:'bookmark_border',name:'Saver',d:'Bookmark pertama'},{id:'bm5',mi:'bookmark',name:'Bookworm',d:'5 bookmark'},{id:'bm20',mi:'bookmarks',name:'Library',d:'20 bookmark'},{id:'note1',mi:'edit_note',name:'Note Taker',d:'Catatan pertama'},{id:'note10',mi:'description',name:'Journal',d:'10 catatan'},{id:'game1',mi:'sports_esports',name:'First Game',d:'Main game pertama'},{id:'game5',mi:'games',name:'Gamer',d:'5 games dimainkan'},{id:'listen1',mi:'headphones',name:'Good Ear',d:'Listening pertama'},{id:'listen10',mi:'graphic_eq',name:'Pro Listener',d:'10 listening'},{id:'scramble1',mi:'shuffle',name:'Unscrambler',d:'Scramble pertama'},{id:'scramble10',mi:'sort_by_alpha',name:'Word Wizard',d:'10 scramble'},{id:'sent1',mi:'construction',name:'Word Smith',d:'Sentence builder pertama'},{id:'sent10',mi:'edit',name:'Writer',d:'10 sentence builder'},{id:'streak3',mi:'local_fire_department',name:'3-Day',d:'Streak 3 hari'},{id:'streak7',mi:'whatshot',name:'7-Day',d:'Streak 7 hari'},{id:'streak14',mi:'emoji_events',name:'14-Day',d:'Streak 14 hari'},{id:'streak30',mi:'shield',name:'30-Day',d:'Streak 30 hari'},{id:'pomodoro',mi:'timer',name:'Focus Time',d:'Selesaikan Focus Learn'},{id:'mcq10',mi:'verified',name:'MCQ Expert',d:'10 quiz MCQ'},{id:'perfect',mi:'verified_user',name:'Perfectionist',d:'Quiz sempurna'}];
function getAv(){var a=localStorage.getItem(LSK('avatar'));if(!a)return DEFAULT_AVATAR;if(String(a).indexOf('gif:')===0){var idx=parseInt(String(a).slice(4));if(isNaN(idx)||idx<0||idx>=AVATARS_GIF.length)return DEFAULT_AVATAR;return a;}return a;}
function renderAvatar(av,size){size=size||'1.2rem';if(String(av).indexOf('gif:')===0){var idx=parseInt(String(av).slice(4));var g=AVATARS_GIF[idx]||AVATARS_GIF[0];return '<span style="font-size:'+size+'">'+g.emoji+'</span>';}return '<span style="font-size:'+size+'">'+esc(av)+'</span>';}
function refreshAv(){var av=getAv();var html=renderAvatar(av,'1.2rem');var hAv=$('h-av');if(hAv)hAv.innerHTML=html;var navAv=$('nav-av');if(navAv)navAv.innerHTML=html;var pfAv=$('pf-av');if(pfAv)pfAv.innerHTML=renderAvatar(av,'3.5rem');}
...
function openProfile(){var s=sess();refreshAv();$('pf-name').textContent=s?s.name:'-';$('pf-id').textContent=s?s.id.toUpperCase():'-';$('pf-class').textContent=s?(s.class||'').toUpperCase():'-';var st=getStats();$('pf-login').textContent=st.loginStreak||0;$('pf-daily').textContent=st.dailyStreak||0;$('pf-parts').textContent=countParts().dn;var dc=getDaily().count>0;$('pf-dc').innerHTML='<span>Daily Challenge</span><b style="color:'+(dc?'var(--p)':'var(--danger)')+'">'+(dc?'Done':'Not yet')+'</b>';var statsHtml='';statsHtml+=statCard('menu_book','Materi',countParts().t);statsHtml+=statCard('flag','Part Selesai',countParts().dn);statsHtml+=statCard('school','Unit Selesai',countUnitsDone());statsHtml+=statCard('repeat','Pengulangan',countRepeat());var ps=$('pf-stats');if(ps)ps.innerHTML=statsHtml;var l=getAch();$('pf-ach-grid').innerHTML=ACH_LIST.map(function(a){var got=l.indexOf(a.id)>=0;return '<div class="ach'+(got?'':' lk')+'" title="'+a.name+' - '+a.d+'"><span class="material-icons-round">'+a.mi+'</span><b>'+a.name+'</b></div>';}).join('');var cur=getAv();var html='';AVATARS.forEach(function(a){html+='<div class="av'+(a===cur?' sel':'')+'" onclick="pickAvatar(\''+a+'\')">'+a+'</div>';});AVATARS_GIF.forEach(function(g,idx){var key='gif:'+idx;html+='<div class="av '+g.cls+(key===cur?' sel':'')+'" onclick="pickAvatar(\''+key+'\')" title="'+g.label+'"><span class="av-inner" style="font-size:1.8rem">'+g.emoji+'</span><span class="av-label">'+g.label+'</span></div>';});$('pf-grid').innerHTML=html;profTab('data');$('ov-profile').classList.add('on');}
...
function renderGrammarUnit(){var fu=findUnit(curUnit);if(!fu||fu.cat!=='grammar'){$('gu-title').textContent='Not found';return;}activeCat='grammar';curCat='grammar';$('gu-title').textContent=cleanTitle(fu.unit.title);var body=$('gu-body');body.innerHTML='';var unit=fu.unit;var materi=unit.materi||'Grammar rule belum tersedia.';var exercises=unit.exercise||[];body.innerHTML+='<div class="grammar-materi"><h4><span class="material-icons-round">auto_stories</span> Materi</h4><div>'+formatMateri(materi)+'</div></div>';if(exercises.length){var exHtml='<div class="sec-t"><span class="material-icons-round">edit_note</span> Exercise <span class="tag full" style="margin-left:.3rem">'+exercises.length+' soal</span></div>';exercises.forEach(function(ex,i){exHtml+=renderGrammarExercise(ex,i);});exHtml+='<button class="btn" onclick="checkGrammarExercises()" style="margin-top:.5rem"><span class="material-icons-round">check_circle</span> Cek Jawaban</button>';exHtml+='<div id="gu-result" style="margin-top:.7rem"></div>';body.innerHTML+=exHtml;}else{body.innerHTML+='<div class="sec-t"><span class="material-icons-round">edit_note</span> Exercise Otomatis</div>';body.innerHTML+='<div class="tx-m" style="font-size:.76rem;margin-bottom:.5rem">Exercise dibuat otomatis dari vocab & speaking materi.</div>';body.innerHTML+=renderAutoGrammarExercise();body.innerHTML+='<button class="btn" onclick="checkGrammarExercises()" style="margin-top:.5rem"><span class="material-icons-round">check_circle</span> Cek Jawaban</button>';body.innerHTML+='<div id="gu-result" style="margin-top:.7rem"></div>';}sendLog('Buka Grammar',curUnit,{kategori:'grammar'});}
function formatMateri(txt){var s=esc(txt);s=s.replace(/\*\*(.+?)\*\*/g,'<code>$1</code>');s=s.replace(/\n/g,'<br>');return s;}
function renderGrammarExercise(ex,i){var html='<div class="grammar-exercise" data-idx="'+i+'" data-answer="'+esc(ex.answer||'')+'">';html+='<div class="ex-q">';if(ex.type==='fill'){var parts=(ex.q||'').split('___');html+=esc(parts[0]||'')+'<input type="text" class="gu-input" data-idx="'+i+'" placeholder="...">'+(esc(parts[1]||''));}else if(ex.type==='choose'){html+=esc(ex.q||'')+'<div style="margin-top:.5rem;display:flex;flex-direction:column;gap:.3rem">';(ex.opts||[]).forEach(function(o,j){html+='<label style="display:flex;align-items:center;gap:.4rem;padding:.4rem .6rem;background:var(--sf2);border-radius:var(--r-sm);cursor:pointer"><input type="radio" name="gu-'+i+'" value="'+esc(o)+'" class="gu-radio" data-idx="'+i+'" style="width:auto">'+esc(o)+'</label>';});html+='</div>';}else if(ex.type==='arrange'){html+=esc(ex.q||'')+'<input type="text" class="gu-input" data-idx="'+i+'" placeholder="Susun kalimat..." style="width:100%">';}else{html+=esc(ex.q||'')+'<input type="text" class="gu-input" data-idx="'+i+'" placeholder="Jawaban...">';}html+='</div></div>';return html;}
function renderAutoGrammarExercise(){var words=allWords();var sents=allSentences();var exercises=[];var w1=shuffle(words.slice()).slice(0,3);w1.forEach(function(w){exercises.push({type:'fill',q:'The Indonesian meaning of "'+w.en+'" is ___.',answer:w.id.toLowerCase()});});if(sents.length){var s1=shuffle(sents.slice()).slice(0,2);s1.forEach(function(sent){var toks=sent.split(' ');if(toks.length>3){var target=toks[Math.floor(Math.random()*Math.min(toks.length,5))].replace(/[.,!?]/g,'');if(target.length>2){var opts=[target];var pool=words.map(function(w){return w.en.toLowerCase();}).filter(function(w){return w!==target.toLowerCase()&&w.length>2;});while(opts.length<3&&pool.length){var r=pool[Math.floor(Math.random()*pool.length)];if(opts.indexOf(r)<0)opts.push(r);pool=pool.filter(function(p){return p!==r;});}var q=sent.replace(new RegExp('\\b'+target.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\b','i'),'___');exercises.push({type:'choose',q:'Fill in the blank: '+q,opts:shuffle(opts),answer:target.toLowerCase()});}}});}window._guAuto=exercises;var html='';exercises.forEach(function(ex,i){html+='<div class="grammar-exercise" data-idx="'+i+'" data-answer="'+esc(ex.answer||'')+'">';html+='<div class="ex-q">';if(ex.type==='fill'){var parts=(ex.q||'').split('___');html+=esc(parts[0]||'')+'<input type="text" class="gu-input" data-idx="'+i+'" placeholder="...">'+(esc(parts[1]||''));}else if(ex.type==='choose'){html+=esc(ex.q||'')+'<div style="margin-top:.5rem;display:flex;flex-direction:column;gap:.3rem">';(ex.opts||[]).forEach(function(o){html+='<label style="display:flex;align-items:center;gap:.4rem;padding:.4rem .6rem;background:var(--sf2);border-radius:var(--r-sm);cursor:pointer"><input type="radio" name="gu-auto-'+i+'" value="'+esc(o)+'" class="gu-radio" data-idx="'+i+'" style="width:auto">'+esc(o)+'</label>';});html+='</div>';}html+='</div></div>';});return html;}
function checkGrammarExercises(){var exes=document.querySelectorAll('.grammar-exercise');var correct=0,total=exes.length;exes.forEach(function(el){var ans=(el.getAttribute('data-answer')||'').toLowerCase().trim();var input=el.querySelector('.gu-input');var radio=el.querySelector('.gu-radio:checked');var userAns='';if(input)userAns=(input.value||'').toLowerCase().trim();else if(radio)userAns=(radio.value||'').toLowerCase().trim();var ok=userAns&&userAns===ans;if(ok){correct++;el.style.borderColor='var(--p)';el.style.background='var(--p-light)';}else{el.style.borderColor='var(--danger)';el.style.background='rgba(239,68,68,.08)';}});var pct=total?Math.round(correct/total*100):0;var grade=pct>=80?'A':pct>=65?'B':pct>=50?'C':'D';var r=$('gu-result');r.innerHTML='<div class="sr" style="background:var(--p-light)"><span style="font-weight:700">Hasil</span><b>'+correct+'/'+total+' ('+pct+'%) - Grade '+grade+'</b></div>';sfx(pct>=50?'ok':'bad');if(pct>=70)confetti();sendLog('Grammar Exercise',curUnit+' = '+correct+'/'+total,{kategori:'grammar'});if(pct>=70)markDone(curUnit,'exercise');}
...
function wrapTtsWords(text,lineIdx){var words=String(text||'').split(/(\s+)/);var html='';var wordIdx=0;words.forEach(function(w){if(/^\s+$/.test(w)){html+=w;return;}if(!w)return;html+='<span class="tts-word" data-line="'+lineIdx+'" data-widx="'+(wordIdx++)+'" onclick="ttsSpeakWord('+lineIdx+','+(wordIdx-1)+')">'+esc(w)+'</span>';});return html;}
function ttsSpeakWord(lineIdx,widx){if(!window._tts||!window._tts[lineIdx])return;var sentence=window._tts[lineIdx];var words=String(sentence).split(/\s+/).filter(Boolean);var word=words[widx];if(!word)return;speakWord(word);document.querySelectorAll('.tts-word').forEach(function(el){el.classList.remove('active');});var el=document.querySelector('.tts-word[data-line="'+lineIdx+'"][data-widx="'+widx+'"]');if(el){el.classList.add('active');setTimeout(function(){el.classList.remove('active');},800);}}
...
/* =====================================================
   AEC MV2 — js.js v5.3 FINAL
   PART 4/4: TTS, Games, Focus, Login, Init
   ===================================================== */
...
function highlightWordInLine(lineIdx,charIdx,charLen){if(charIdx<0){document.querySelectorAll('.tts-word.active').forEach(function(el){el.classList.remove('active');});return;}var sentence=window._tts[lineIdx]||'';var beforeText=String(sentence).substring(0,charIdx);var wordCount=beforeText.split(/\s+/).filter(Boolean).length;document.querySelectorAll('.tts-word.active').forEach(function(el){el.classList.remove('active');});var el=document.querySelector('.tts-word[data-line="'+lineIdx+'"][data-widx="'+wordCount+'"]');if(el){el.classList.add('active');el.scrollIntoView({block:'center',behavior:'smooth'});}}
...
window.addEventListener('message',function(e){if(e.data&&e.data.type==='aec-game-back'){closeGM();var d=e.data;if(d.ok!=null&&d.total!=null){var pct=d.total?Math.round(d.ok/d.total*100):0;confetti();sfx('done');showReward(d.source||'game',d.ok,d.total,pct);sendGameSession(d.type||d.source||'game',d.ok,d.total,d.ok*10);if(d.ok===d.total&&d.total>=5)checkAch('perfect','Perfectionist');}}});
...
function pomoStart(){if(pomoRun){clearInterval(pomoRun);pomoRun=null;stopFocusGuard();}else{closeOv('ov-focus');doFS();startFocusGuard();pomoRun=setInterval(function(){pomoLeft--;updatePomo();if(pomoLeft<=0){clearInterval(pomoRun);pomoRun=null;pomoLeft=25*60;updatePomo();sfx('done');checkAch('pomodoro','Focus Time');stopFocusGuard();if(document.fullscreenElement&&document.exitFullscreen)document.exitFullscreen();}},1000);}updatePomo();}
...
function renderGuide(){var g=lang==='id'?[{i:'menu_book',t:'Learn: Vocabulary, Speaking, Grammar'},{i:'quiz',t:'Practice: latihan vocab/speaking/grammar'},{i:'mic',t:'Speak: latihan bicara STT'},{i:'language',t:'Extra: English Tools, Games, Overview Siswa'},{i:'timer',t:'Focus Learn: tombol melayang, klik untuk fokus'},{i:'fullscreen',t:'Ketuk layar / tombol fullscreen'},{i:'person',t:'Klik nama = profil'}]:[{i:'menu_book',t:'Learn: Vocabulary, Speaking, Grammar'},{i:'quiz',t:'Practice: vocab/speaking/grammar drills'},{i:'mic',t:'Speak: STT speaking practice'},{i:'language',t:'Extra: English Tools, Games, Overview'},{i:'timer',t:'Focus Learn: floating button'},{i:'fullscreen',t:'Tap screen / fullscreen button'},{i:'person',t:'Tap name = profile'}];$('guide-list').innerHTML=g.map(function(x){return '<li style="display:flex;gap:.7rem;margin:.6rem 0;font-size:.84rem;align-items:flex-start"><span class="material-icons-round" style="color:var(--p);font-size:19px;flex-shrink:0">'+x.i+'</span><span>'+x.t+'</span></li>';}).join('');}
var obStep=0;
function openOnb(){obStep=0;showOB();$('ov-onb').classList.add('on');}
function showOB(){var steps=[{mi:'waving_hand',t:'Welcome to AEC!',d:'Your English learning platform'},{mi:'menu_book',t:'Learn',d:'Vocabulary, Speaking & Grammar'},{mi:'quiz',t:'Practice',d:'Drill your skills'},{mi:'mic',t:'Speak Live',d:'STT speaking practice'},{mi:'timer',t:'Focus Learn',d:'Floating button for focus'},{mi:'emoji_events',t:'Earn Badges',d:'31 achievements'}];var s=steps[obStep];$('ob-body').innerHTML='<div class="tc" style="padding:1rem 0"><span class="material-icons-round" style="font-size:3.6rem;color:var(--p);display:block;margin-bottom:.7rem">'+s.mi+'</span><h3>'+s.t+'</h3><p class="tx-m">'+s.d+'</p></div>';$('ob-next').innerHTML=(obStep<steps.length-1?'Next <span class="material-icons-round">arrow_forward</span>':'<span class="material-icons-round">rocket_launch</span> Start');}
function obNext(){obStep++;if(obStep>=6){closeOv('ov-onb');localStorage.setItem(LSK('onb'),'1');return;}showOB();}
async function login(){var id=$('in-id').value.trim().toLowerCase(),pin=$('in-pin').value.trim(),err=$('login-err'),errL=$('login-err-load');err.classList.remove('on');errL.classList.remove('on');if(!id||!pin){$('login-err-msg').textContent='Isi ID dan PIN!';err.classList.add('on');return;}try{await loadAll();}catch(e){$('login-err-load-msg').textContent='Gagal memuat data: '+e.message;errL.classList.add('on');return;}var st=(D.students||[]).find(function(s){return s.id&&String(s.id).toLowerCase()===id&&String(s.pin)===pin;});if(!st){$('login-err-msg').textContent='ID atau PIN salah!';err.classList.add('on');sfx('bad');return;}if(st.active===false){$('login-err-msg').textContent='Akun tidak aktif. Hubungi admin.';err.classList.add('on');sfx('bad');return;}localStorage.setItem(LSK('aecSession'),JSON.stringify({id:st.id,name:st.name,class:st.class,loginTime:Date.now(),durationDays:D.settings.sessionDuration||10}));sendLog('Login','Berhasil login',{kategori:'-'});touchLoginStreak();refreshAv();if(!localStorage.getItem(LSK('onb')))openOnb();showDCBalloon();location.hash='#learn';route();}
...
async function checkSession(){var sd=localStorage.getItem(LSK('aecSession'));loadAll().catch(function(e){ERR('preload gagal:',e.message);});if(!sd){setTimeout(function(){$('splash').classList.add('hide');},2600);showLogin();return;}try{var s=JSON.parse(sd);if(Date.now()-s.loginTime>s.durationDays*86400000){localStorage.removeItem(LSK('aecSession'));setTimeout(function(){$('splash').classList.add('hide');},2600);showLogin();return;}var cnEl=$('c-n');if(cnEl)cnEl.textContent=(s.name||'—');$('c-i').textContent=s.id.toUpperCase();updateCountdown();setInterval(updateCountdown,60000);touchLoginStreak();refreshAv();try{await loadAll();setTimeout(function(){$('splash').classList.add('hide');},2600);if(!localStorage.getItem(LSK('onb')))openOnb();showDCBalloon();route();}catch(e){setTimeout(function(){$('splash').classList.add('hide');},2600);route();}}catch(e){setTimeout(function(){$('splash').classList.add('hide');},2600);showLogin();}}
